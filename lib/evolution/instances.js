import { evolutionFetch, getEvolutionConfig, isEvolutionBusinessApi } from './client';

export async function createInstance(instanceName) {
  if (isEvolutionBusinessApi()) {
    const token = process.env.EVOLUTION_BUSINESS_TOKEN || '';
    const number = process.env.EVOLUTION_PHONE_NUMBER_ID || '';
    const businessId = process.env.EVOLUTION_BUSINESS_ID || '';
    if (!token || !number || !businessId) {
      throw new Error(
        'WhatsApp Cloud API: defina EVOLUTION_BUSINESS_TOKEN, EVOLUTION_PHONE_NUMBER_ID e EVOLUTION_BUSINESS_ID no .env'
      );
    }
