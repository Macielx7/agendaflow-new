...
import { handleAppointmentButtonResponse } from './buttonActions';
...
export async function handleEvolutionWebhook(tenantId, body) {
  ...
  if (event.includes('MESSAGE') || event.includes('POLL') || event.includes('UPSERT')) {
    const confirmationHandled = await handleAppointmentButtonResponse(tenantId, body).catch((err) => {
      console.error('[whatsapp webhook reply]', err.message);
      return false;
    });

    if (!confirmationHandled) {
      const aiResult = await processIncomingMessage(tenantId, body).catch((err) => {
        console.error('[ai webhook]', err.message);
        return { handled: false, reason: 'ERROR', error: err.message };
      });
      if (aiResult && !aiResult.handled) {
        console.warn('[ai webhook skip]', aiResult.reason, aiResult.error || '');
      }
    }
  }
