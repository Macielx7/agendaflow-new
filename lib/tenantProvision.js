...
export async function provisionTenant({
  ...
  const { ensureWhatsappDefaults } = await import('./whatsapp/templates');
  await ensureWhatsappDefaults(tenant.id);

  const { ensureAiDefaults } = await import('./ai/ensureDefaults');
  await ensureAiDefaults(tenant.id);
