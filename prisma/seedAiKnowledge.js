...
import { buildKnowledgeEntries } from '../lib/ai/knowledgeTemplates.js';
import { normalizeText, extractKeywords } from '../lib/ai/normalize.js';
import { buildSystemFaqEntries } from '../lib/ai/defaults.js';

async function getClinicInfo(prisma, tenantId) {
...
async function syncKnowledgeToFaq(prisma, tenantId, knowledge) {
...
export async function seedAiKnowledgeForTenant(prisma, tenantId, { enableAi = true } = {}) {
  let aiSettings = await prisma.aiSettings.findUnique({ where: { tenantId } });
  if (!aiSettings) {
    aiSettings = await prisma.aiSettings.create({ data: { tenantId } });
  }

  if (enableAi) {
    await prisma.aiSettings.update({
      where: { id: aiSettings.id },
      data: { enabled: true, autoReplyEnabled: true },
    });
  }
  ...
  if (systemCount === 0) {
    const clinic = await getClinicInfo(prisma, tenantId);
    const entries = buildSystemFaqEntries(tenantId, clinic);
    await prisma.faqEntry.createMany({ data: entries });
  }

  const knowledgeCount = await prisma.knowledgeBase.count({ where: { tenantId } });
  if (knowledgeCount === 0) {
    const clinic = await getClinicInfo(prisma, tenantId);
    ...
    for (const entry of entries) {
      ...
      await syncKnowledgeToFaq(prisma, tenantId, knowledge);
    }
  }
